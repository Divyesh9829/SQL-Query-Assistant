...
class ChatBot:
    ...
    def __init__(self):
        openai.api_key = os.getenv("OPENAI_API_KEY")  # Load OpenAI API key from environment variable
    ...
    def respond(chatbot: List, message: str, chat_type: str, app_functionality: str) -> Tuple:
        ...
        if app_functionality == "Chat":
            ...
            # If we want to use langchain agents for Q&A with our SQL DBs that were created from CSV/XLSX files.
                ...
                # Initialize the database toolkit with your database connection
                toolkit = SQLDatabaseToolkit(llm=APPCFG.langchain_llm, db=db)
                agent_executor = create_sql_agent(
                    APPCFG.langchain_llm,toolkit=toolkit, verbose=True)
                ...
                response = openai.Embedding.create(
                    input=message,
                    model=APPCFG.embedding_model_name  # Use the embedding model from OpenAI
                )
                ...
                llm_response = openai.ChatCompletion.create(
                    model=APPCFG.model_name,  # Use the model directly from OpenAI
                    messages=messages
                )
